...
A Claude Code plugin that reads your app's user-facing surfaces, decides whether a spotlight tour is worth building, and — when it is — generates a short, instrumented, replayable Driver.js tour with a human-gated anchor pass.

---

## The differentiator: earn the tour first

Most onboarding tools assume a tour is always the answer. Vibe-Walk does not.

Phase 1 runs entirely on your codebase — orientation docs, route surface, component composition, existing onboarding code — and produces a real verdict before anything is built. "Don't build a tour here" is a first-class output, weighted the same as "build one." A tour layered on an already-intuitive UI trains users to dismiss future guidance. A tour on a blank-canvas first-run surface has lower ROI than fixing the empty state. A power-user tool doesn't need a spotlight tour at all.

The plugin checks for these conditions and tells you honestly. Sherpa — the Vibe-Walk persona — is a guide who will recommend against the summit when the conditions are wrong. That's what makes the "build" verdict trustworthy when it comes.
...
Run `/vibe-walk:discover` in the target app's repo. The plugin reads:
...
3. A named aha-moment candidate — the single action that makes a new user say "this is worth it." Step 1 of any built tour routes here.
...
**Analytics wiring** — six events bound to Driver.js substrate hooks, emitted into your existing analytics system:
- `tour_started`
- `tour_step_viewed`
- `tour_step_advanced`
- `tour_skipped`
...
A `TOUR_ANALYTICS.md` is generated naming the events, the host activation event, and the 7d/14d attribution windows. The success criterion is downstream activation, not tour completion — the latter is a trap metric.
...
All session and friction logging is local-only under `~/.claude/plugins/data/vibe-walk/`. Nothing leaves the machine. You can inspect or delete the directory at any time; the plugin will still function, it just loses its session memory for self-evolution.

---

## Install

Via the stable marketplace:

```
/plugin install estevanhernandez-stack-ed/vibe-plugins
```

Via the canary channel (latest `main` from the solo repo):

```
/plugin install estevanhernandez-stack-ed/Vibe-Walk
```

---

*By 626Labs LLC — [estevanhernandez-stack-ed](https://github.com/estevanhernandez-stack-ed)*
