...
Emits three files for the Driver.js / Shape A (drop-in module) path:
  ...
  spotlightSteps.ts
    ...
    Step copy is referenced via a tiny t() helper that reads from
    spotlight.i18n.json with inline English fallbacks — see i18n below.
  ...
  spotlight.i18n.json
    Sibling translation file keyed by step name + field. Schema:
      "spotlight.step.<anchor>.title":       "<title text>"
      "spotlight.step.<anchor>.description": "<description text>"
    The emitted spotlightSteps.ts imports this file and references each key
    via a t(key, fallback) helper. To localize, hosts either replace the
    JSON contents or merge keys into their existing i18n bundle. Removing
    the file does not break the tour — the inline fallbacks remain.
...
def emit_module(build_plan: dict) -> dict:
    ...
    # Build the i18n dict first so the steps file can reference its keys
    # while keeping inline English fallbacks intact.
    step_copy = _build_step_copy(stops, audience)
    i18n_content = _emit_i18n_json(step_copy)

    # Emit the three files
    steps_content = _emit_steps_file(stops, step_copy, app_slug)
    ...
    return {
        "files": {
            ...
            "spotlight.i18n.json": i18n_content,
        },
        "warnings": warnings,
    }
...
def _emit_steps_file(stops: list[dict], step_copy: list[dict], app_slug: str) -> str:
    """
    ...
    Title + description go through a tiny t() helper that reads from
    spotlight.i18n.json (sibling file) with inline English fallbacks.
    Removing or breaking the JSON does NOT blank the tour — fallbacks remain.
    ...
    lines: list[str] = [
        ...
        "// Step copy is referenced via t() with fallbacks; live strings in",
        "// spotlight.i18n.json. Replace the JSON to localize.",
        "import type { DriveStep } from 'driver.js';",
        "import spotlightI18n from './spotlight.i18n.json';",
        "",
        "// Local i18n helper — reads from spotlight.i18n.json, falls back to inline English.",
        "// To localize: replace spotlight.i18n.json contents, or merge keys into your i18n bundle.",
        "type I18nDict = Record<string, string>;",
        "const _i18n: I18nDict = spotlightI18n as I18nDict;",
    ...
    for i, stop in enumerate(stops):
        ...
        title_key = _i18n_key(anchor, "title")
        description_key = _i18n_key(anchor, "description")
        # Escape single quotes for safe TS string literals in fallback args
        title_fallback = step_copy[i]["title"].replace("'", "\\'")
        description_fallback = step_copy[i]["description"].replace("'", "\\'")

        lines.append("  {")
        lines.append(f"    element: '[data-tour=\"{anchor}\"]',")
        lines.append("    popover: {")
        lines.append(f"      title: t('{title_key}', '{title_fallback}'),")
        lines.append(f"      description: t('{description_key}', '{description_fallback}'),")
...
# ---------------------------------------------------------------------------
# i18n helpers — step copy extraction + JSON emitter
# ---------------------------------------------------------------------------

def _i18n_key(anchor: str, field: str) -> str:
    """
    Build the i18n key for a step field.
    Schema: spotlight.step.<anchor>.<field> where field ∈ {title, description}.
    """
    return f"spotlight.step.{anchor}.{field}"


def _build_step_copy(stops: list[dict], audience: str) -> list[dict]:
    """
    Build per-step copy (title + description) using the audience register.
    Returns a list parallel to stops; each entry carries:
      "_anchor"     — anchor name (for i18n key construction)
      "title"       — generated title
      "description" — generated description

    Centralizes copy generation so the steps emitter and the i18n emitter
    pull from the same source of truth (no drift between them).
    """
...
def _emit_i18n_json(step_copy: list[dict]) -> str:
    """
    Emit spotlight.i18n.json — a flat key/value map of i18n keys to English copy.

    Schema:
      {
        "spotlight.step.<anchor>.title":       "<title>",
        "spotlight.step.<anchor>.description": "<description>"
      }

    Hosts localize by replacing this file's contents (or merging keys into
    their i18n bundle). Order matches the steps emission order — readability
    only; runtime lookup is by key, order doesn't matter.
    """
    i18n_dict: dict = {}
    for entry in step_copy:
        anchor = entry["_anchor"]
        i18n_dict[_i18n_key(anchor, "title")] = entry["title"]
        i18n_dict[_i18n_key(anchor, "description")] = entry["description"]
    return json.dumps(i18n_dict, indent=2, ensure_ascii=False) + "\n"
